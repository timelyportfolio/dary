...
convert_level <- function(l, idname = "name") {
  if(is.list(l) && !is.null(names(l))) {
    mapply(
      function(ll, nm) {
        if(!is.list(ll)) {
          ll <- lapply(ll, function(l3) {x <- list(l3); names(x) <- idname; x})
        }
        x <- list(
          name = nm,
          children = ll
...
convert_dict_flat <- function(dict) {
  rows <- unlist(lapply(readLines(textConnection(as.yaml(dict))),strsplit,split="  "), recursive = FALSE)
  ...
  l <- lapply(
    ...
    function(rw) {
      ...
      if(grepl(x=tail(rw,1), pattern=":")) {
        empties <- Filter(function(x) {x == ""}, rw)
      ...
      df <- data.frame(
        structure(
          as.list(c(gsub(rw, pattern="^(- )", replacement = ""), pad)),
      ...
      if(!is.null(rw2)) {
        rbind(
          ...
          data.frame(
            structure(
              as.list(c(rw2, head(pad, length(pad)-1))),
              names = paste0("level",1:height)
            ),
            stringsAsFactors = FALSE
          )
